...
--
-- Phase 2 kept exactly one `files` row per user (`files_one_per_user_uq`) as the
-- cloud equivalent of the single local `geojson_data` record. Phase 3 adds the
-- "My Files" browser, so a user may now own many rows, each a named file. The
...
-- ============================================================================
drop index if exists public.files_one_per_user_uq;
...
-- Nothing else changes: `geojson`, `backup_geojson`, `created_at`, `updated_at`,
-- the `updated_at` trigger, and the owner-only RLS policies from 0001 already
-- cover multiple rows per user (every policy is scoped to `auth.uid() = user_id`).
