...
> **Status:** Phases 0–5 complete (seams + auth + remote file/settings round-trip + multi-file "My Files" + the Node server layer with account deletion + the account area & compliance pages), the **UI/UX revamp of every cloud surface is complete (2026-07-06)**, and **Phase 6 — monetisation — is built and fully verified in Stripe test mode against non-prod (2026-07-24, ADR-031):** `plans` + `user_plans` + the storage-quota trigger (`0005`), the Node `/api/v1/billing` Stripe checkout/portal/webhook layer, and the client billing UI (real plan/usage + Upgrade CTA + live Customer Portal) — the full subscription lifecycle (checkout→webhook grant, portal, quota gate, cancel, test-clock renewal + dunning) confirmed green. The plan-tier product shape is settled (**ADR-032, 2026-07-25:** four tiers — Guest / Free / Basic / Pro — differentiated by storage + file-count only). **Now — Phase 7, re-sliced again 2026-07-27 into five slices with verification last:** **7a** entitlement-completion build — **built + applied 2026-07-26** (the file-count lever now enforced, the `early_access → free` rename, monotonic seeds, hidden plans, and the two Phase 6 billing follow-ups) → **7b** the **large-file fix** — a 43 MB import crashes the whole Supabase project, so the GeoJSON blob moves out of `jsonb` into **Supabase Storage** ([ADR-035](02-decisions.md#adr-035--the-geojson-blob-moves-to-supabase-storage-user_files-becomes-a-metadata-row)); the biggest single change left before beta — **paused 2026-07-31, re-designed 2026-08-01 as [ADR-037](02-decisions.md#adr-037--a-file-is-a-snapshot-plus-deltas-whole-document-writes-are-retired): a file is a *snapshot plus deltas*.** The Storage move survives as half the design; the whole-document write cadence is replaced by feature-level deltas in Postgres, so the cost of an edit no longer depends on the size of the file. No vendor change — the review confirmed this was a data-model problem. Signed upload URLs and per-write quota checks are withdrawn, restoring client-direct writes. **Built 2026-08-01; first smoke tests pass 2026-08-02:** `0006` **applied to non-prod with all SQL checks passing**; the delta model, both storage backends, the seam, the edit sites and the Web Lock (dialog + read-only banner) are built and the app build is clean. **The 43 MB file now edits without crashing or server errors** — the failure the slice exists to remove — and the two-tab lock behaves. **Non-prod was reset to zero on 2026-08-02** (all data, accounts and Storage objects) with the migration chain replayed from empty. Still open: the account-deletion Storage sweep is unbuilt, and an occasional "invalid feature" error on refresh after rapid point creation. Two design corrections came out of building it — **the watermark is a scalar and so has the same trap as the delete predicate** (advance to the top of the unbroken run, never to `max`), and **a checkpoint needs a compare-and-set** because the delete rule protects the deltas but not the snapshot object ([ADR-037 addendum 2](02-decisions.md#adr-037--a-file-is-a-snapshot-plus-deltas-whole-document-writes-are-retired)) → **7c** loose ends → **7d** polish → **7e** real-account verification & beta sign-off. **Then Phase 8 — free beta / early access, the production-provisioning gate (ADR-014); Phase 9 — go-live** — everything so far has been on non-prod. See [`05-worklog.md`](05-worklog.md).
...
```
   ...
   │   snapshot + deltas,    │                    │   Storage — the SNAPSHOT      │
   │   backup_geojson_data   │                    │     (whole doc, at checkpoints)│
   │ localStorage            │                    │   Postgres — DELTAS (per edit)│
   │   templates, bookmarks, │                    │     metadata, settings, plans │
   │   settings, etc.        │                    │   Auth (JWT)                  │
   └─────────────────────────┘                    └───────────────────────────────┘

   Existing Node API (format conversions, Turnstile session) — unchanged for both.

   Both paths store a file the same way: a file = snapshot + deltas (ADR-037).
```
...
- **The seam(s)** — the storage-provider abstraction that lets the app swap local vs remote backends without knowing which is in use. Two of them: the **File seam** (the active GeoJSON document) and the **Settings seam** (templates/bookmarks/prefs). See [`01-architecture.md`](01-architecture.md#4-the-storage-provider-seam).
- **Snapshot / delta / checkpoint** ([ADR-037](02-decisions.md#adr-037--a-file-is-a-snapshot-plus-deltas-whole-document-writes-are-retired)) — **a file is a snapshot plus every delta recorded after it; neither half is the file on its own.** The **snapshot** is the whole document, written rarely; a **delta** is one changed feature, written on every edit; a **checkpoint** is the moment a fresh snapshot is written and the deltas it covers are cleared. Applies to both storage paths.
- **One file, one place at a time** — editing a file from **multiple tabs or devices is not supported** in v1 (stated in the UI). A Web Lock blocks a second tab in the same browser; cross-device is warned rather than blocked, and nothing is destroyed either way. A server-side lease is backlogged. See [ADR-037's addendum](02-decisions.md#adr-037--a-file-is-a-snapshot-plus-deltas-whole-document-writes-are-retired).
