...
> **Status:** Phases 0–5 complete (seams + auth + remote file/settings round-trip + multi-file "My Files" + the Node server layer with account deletion + the account area & compliance pages), and the **UI/UX revamp of every cloud surface is complete (2026-07-06)** — landing, split-layout `/login` with a 4-provider OAuth grid, in-app chrome, `/files`, the sidebar'd account page. **Next: the user's real-account verification pass on staging, then the pre-Phase-6 loose ends; Phase 6 (free beta / early access) is the production-provisioning gate (ADR-014)** — everything so far has been on non-prod. See [`05-worklog.md`](05-worklog.md).
...
The **UI/UX revamp (rollout near-term step 3) is complete (2026-07-06)**: cloud account surfaces are dedicated routes (ADR-029) — the landing (Phase 7 Stage 1 pulled forward), a split-layout `/login` with an OAuth provider grid (Google/GitHub/Microsoft/Facebook — only Google configured in Supabase so far), `/files`, and an `/account` page with a section sidebar plus Security and Billing panels; in-app chrome (avatar menu + active-file chip); everything audited dark + responsive. **Next:** the user's **real-account verification pass** (consolidated list in the worklog) and the remaining loose ends (OAuth provider config, per-file size limit, legal content), then **Phase 6 (free beta / early access)** — the production-provisioning gate ([ADR-014](02-decisions.md#adr-014--separate-supabase-projects-per-environment-non-prod-set-up-first)): the point where the epic leaves non-prod for a real production Supabase project, gated behind a beta allowlist. See [`05-worklog.md`](05-worklog.md) for the running status.
