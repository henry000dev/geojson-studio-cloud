...
- **Snapshot** — the object at `{user_id}/{file_id}.geojson`. **Deltas** — `public.file_deltas`, one row per feature touched since the snapshot, deduplicated by `(file_id, feature_id)`, emptied at each checkpoint.
...
- **⚠ The multi-session delete rule — non-negotiable, roughly ten lines.** **A checkpoint may only delete deltas it actually incorporated**, never "everything up to now." Without it: two devices open at watermark 10, B writes 11–12, A writes 13, A checkpoints with a document that never contained B's edits and deletes up to 13 — **destroying B's work in both places, silently.** Build this *with* the checkpoint, not after it.
- **Checkpoint policy** — on idle; when the buffer gets **wide** (bulk operations flood it, so `if the delta would be bigger than the snapshot, just snapshot`); on file switch, sign-out, and best-effort on tab close; and **before publish**. Never mid-drawing — edits during an upload land in the buffer above the watermark, which is exactly right. Starting values, tuned later.
...
- **Confirm-as-you-go:** a 43 MB file imports, edits, reloads and exports **without taking the project down**; an edit burst on a 40 MB file uploads **kilobytes, not megabytes**; killing the tab mid-session loses nothing (the deltas are already in the cloud); opening the same file in a second browser reconstructs it correctly from snapshot + deltas; **two sessions editing concurrently do not destroy each other's work**; a file over the bucket limit is refused cleanly; two-account isolation holds on `storage.objects` **and** on `file_deltas` (both new RLS surfaces); account deletion leaves no objects behind; the usage figure on `/account` still matches reality.
...
- **Multi-session UI** ([ADR-037](02-decisions.md#adr-037--a-file-is-a-snapshot-plus-deltas-whole-document-writes-are-retired) *Consequences*). The **data-loss** half is closed in 7b-2 by the "only delete deltas you incorporated" rule; this is the **user-facing** half. Two cheap pieces: **detect-on-write** (compare the watermark already in hand → *"changed in another window — reload"*) and a **same-browser-tab check** via Web Locks or `BroadcastChannel`, which catches the commonest real case — one person with a forgotten second tab — with no server involvement. A **soft lock** (first session holds the write lease; a second opens read-only or takes over, as SharePoint/Excel do) is **held in reserve**: it needs a lease column, a heartbeat and stale-lease handling, and should wait for evidence it is needed. Realistic target: this is a single-user editor, so multi-session means one person on two devices, usually by accident.
