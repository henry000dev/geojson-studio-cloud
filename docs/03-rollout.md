...
> **⏸ PAUSED 2026-07-31 — the remedy is under review.** The user is reconsidering the approach (other providers, edge functions, NoSQL documents) on three grounds: whole-document writes of up to 50 MB per edit burst are not acceptable for a paid product; the quota mechanism is fiddly and thinly supported; and needing prototypes at all is a signal. **Nothing was built — `0006` never existed, so there is nothing to unwind.** See the [ADR-035 third addendum](02-decisions.md#adr-035--the-geojson-blob-moves-to-supabase-storage-user_files-becomes-a-metadata-row) for the reasoning and for what survives a provider change. **The diagnosis is unaffected:** a 43 MB `jsonb` write takes the project down and the blob cannot stay in Postgres, so *something* in this shape still has to happen. **Read the rest of this slice as the Supabase-specific plan of record, valid if the review returns here.** Note the sharpest point for the rethink: the 50 MB-per-burst objection is a **write-granularity** problem (`file_features`, [`04-backlog.md`](04-backlog.md)), not a provider problem — no vendor change reaches it.

- ~~**Two prototypes first — both gate the design**~~ **— DONE (2026-07-30), gate discharged.** Both run against non-prod and torn down clean; scripts in [`../supabase/prototypes/`](../supabase/prototypes/), findings in the [ADR-035 addendum](02-decisions.md#adr-035--the-geojson-blob-moves-to-supabase-storage-user_files-becomes-a-metadata-row). **(2) answered outright — one private bucket, no public-bucket fallback needed:** a cross-table subquery in a `storage.objects` policy works via a `security definer` lookup (the naive form fails on a *grant*, not RLS), costs 0.285 ms, denies non-disclosingly, and revokes immediately. **(1) answered "works, but":** the trigger fires on the real Storage API path and can see the size at `BEFORE INSERT`, but an overwrite is an **upsert** (so the naive trigger double-counts on every autosave and inverts humane-downgrade), the rejection reaches the client as an opaque `500` that ADR-025's retry logic would treat as transient, and orphaned bytes are `UNDETERMINED` with the evidence favouring "yes".
- **Quota enforced by Node-issued signed upload URLs — decided 2026-07-31** ([ADR-035 addendum](02-decisions.md#adr-035--the-geojson-blob-moves-to-supabase-storage-user_files-becomes-a-metadata-row)), promoting the documented fallback over the `storage.objects` trigger. The only option that refuses *before the bytes move* (so the orphan question never arises), returns a clean `4xx` carrying our own message (no retry collision, no dependency on a Postgres SQLSTATE surviving two service layers), and avoids the upsert arithmetic entirely. **Save becomes `mint → PUT to the signed URL → update the row`** — two of three steps stay client-direct, and reads are untouched. Node reads the reworked `user_storage_usage` view with its existing `service_role` client; there is no per-user storage API in Supabase. **Two consequences to build for:** the quota becomes **bounded-overshoot, not exact** (a signed URL authorizes a path, not a byte count — bounded by `file_size_limit` × `max_files` and self-correcting on the next save), and **write authorization moves from RLS to Node**, kept safe by Node building `{user_id}/{file_id}.geojson` from the *verified* `sub` claim rather than from client input.
- **⚠ P3 — one prototype still outstanding, and it shapes the client (not the SQL).** **Is a signed upload URL reusable, or consumed after one upload?** Reusable → mint once per file per session, autosave costs nothing extra. Single-use → every autosave pays a Node round-trip before the write starts, which widens 7b-1's cost-priced coalescing window to 1200 ms and makes the **7c journal a prerequisite rather than a companion**. Not to be assumed — the 2026-07-30 round found three undocumented behaviours in this same API. A ~20-line script; see [`../supabase/prototypes/`](../supabase/prototypes/).
- **New failure class to handle: `401` on mint** while the user has unsaved work on screen — a state that does not exist today. The [ADR-036](02-decisions.md#adr-036--local-storage-buffers-cloud-writes-the-recovery-journal-is-promoted-indexeddb-as-a-staging-area-is-rejected) journal (7c) is the mitigation; until it lands, the client must at minimum refuse to discard the edit and surface a re-authenticate prompt.
...
- **Byte-quota enforcement re-based on `storage.objects.metadata->>'size'`** — the only place that knows the real size, and confirmed populated at `BEFORE INSERT` (2026-07-30). A client-written `size_bytes` column is **rejected as the source of truth** (spoofable by the same replay ADR-033 describes). **File-count enforcement is untouched** — `enforce_file_count` still counts metadata rows. *Shape depends on the open decision above; if the trigger route wins, it must be upsert-aware — see the ADR-035 addendum.*
...
- **Account-deletion sweep — now *mandatory*, not merely the natural home (2026-07-30).** `on delete cascade` does **not** remove storage objects, and the prototypes found that `storage.protect_delete()` rejects any direct SQL `DELETE` on `storage.objects` with `42501` — so **no SQL mechanism can clean those rows either**: not a cascade, not a trigger, not a scheduled job. The `{user_id}/` prefix sweep on the existing `POST /api/v1/account/delete` (Phase 4, already `service_role`), via the **Storage API**, is the only home for it — plus an orphan-reconciliation query as a backstop. The ADR-035 delete ordering (object first, then row) is now enforced by the platform rather than merely recommended: the reverse is impossible. **A compliance obligation, not housekeeping.**
...
- **Cache-control decided deliberately — and it is a per-upload lever (confirmed 2026-07-30).** `cacheControl` is set by the **uploader**, not fixed by the bucket (`curl PUT` defaults to `no-cache`; the dashboard's multipart upload sets `max-age=3600`), so the app chooses it on every write. Short/no cache on owner reads (a just-saved file served stale reads as "my edit disappeared"); for published files this is a **product** call, because revocation was measured immediate on a `no-cache` object and a long `max-age` would likely serve a revoked file until it expires.
